...
grid.arrange(plot_disco, plot_techno)
```
...
grid.arrange(tempogram_alive, tempogram_mistreated, tempo_boxplot)
```
...
grid.arrange(histogram_loudness, boxplot_loudness)
```
...
Taking into account outliers though, the loudness does not differ too much between the two genres. This is visible in the boxplot. The median of the loudness of the techno genre is equal to -8 and the median of disco music lies around -7.5. 
...
grid.arrange(daddy_cool_plot, aftermath_plot)
```
...
ssm_perpetual <- perpetual %>%
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_classic() +
  labs(x = "", y = "") + ggtitle('SSM Perpetual Groove')



grid.arrange(ssm_survive, ssm_perpetual)
...
grid.arrange(chromagram_disco, chromagram_techno)
```
